...
export default function FeatureSectionClient({ features }: { features: Feature[] }) {
	...
	const displayFeatures = features && features.length > 0 ? features : [
		{
			...
			description: "Comprehensive carbon footprint analysis across the entire building lifecycle, from construction to demolition.",
			icon: "leaf",
			order: 1,
			isActive: true
		},
		{
			...
			description: "Advanced energy performance simulation and optimization for new builds and retrofit projects.",
			icon: "zap",
			order: 2,
			isActive: true
		},
		{
			...
			description: "Financial assessment of sustainability investments with long-term ROI calculations and risk analysis.",
			icon: "chart",
			order: 3,
			isActive: true
		},
		{
			...
			description: "Expert guidance on environmental, social, and governance compliance for built environment projects.",
			icon: "shield",
			order: 4,
			isActive: true
		},
		{
			...
			description: "Sustainable procurement solutions and supplier assessment for green building materials and services.",
			icon: "shopping-cart",
			order: 5,
			isActive: true
		},
		{
			...
			description: "Strategic portfolio-wide sustainability planning and optimization for multiple building assets.",
			icon: "building",
			order: 6,
			isActive: true
		}
	...
	return (
		<div className="py-16 md:py-20 bg-white relative overflow-hidden">
			...
			>
				We offer faster and simpler pathways to decarbonise the built environment.
			</motion.h2>
			<motion.p
				initial={{
					opacity: 0,
				...
				{displayFeatures.map((feature, index) => (
					...
					>
						...
						<div className="relative z-20">
							...
							<p className="text-sm md:text-base font-bold text-neutral-800 dark:text-white">{feature.title}</p>
							<p className="text-xs md:text-sm lg:text-base text-neutral-600 dark:text-neutral-400 mt-2 md:mt-4 font-normal">{feature.description}</p>
						</div>
					</motion.div>
				))}
			</div>
